...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(data.store ? "/dashboard" : "/auth/setup-store");
    } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-emerald-900 via-emerald-700 to-emerald-500 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Login to your store dashboard</CardDescription>
        </CardHeader>
        <CardContent>
          {error && (
        ...
        <CardFooter className="justify-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href="/auth/register" className="font-semibold text-emerald-600 hover:underline">Register</Link>
          </p>
        </CardFooter>
      </Card>
    </div>
